...
test("Python Plan-and-Solve 课接入增量上下文个性化练习 family", async () => {
  const { authoredCatalog } = await import("../app/content/catalog.ts");
  const lesson = authoredCatalog.tracks.find(({ id }) => id === "python")?.lessons.find(({ id }) => id === "agent-plan-solve");
  assert.equal(lesson?.familyId, "python-plan-solve-v1");
  const family = (await import("../app/exercises/families.ts")).exerciseFamilies.find(({ id }) => id === lesson.familyId);
  assert.ok(family);
  assert.equal(family.variants.length, 6);
...
test("Python 记忆检索课接入排序与大小写边界个性化练习 family", async () => {
  const { authoredCatalog } = await import("../app/content/catalog.ts");
  const lesson = authoredCatalog.tracks.find(({ id }) => id === "python")?.lessons.find(({ id }) => id === "agent-memory-retrieval");
  assert.equal(lesson?.familyId, "python-memory-retrieval-v1");
  const family = (await import("../app/exercises/families.ts")).exerciseFamilies.find(({ id }) => id === lesson.familyId);
  assert.ok(family);
  assert.equal(family.variants.length, 6);
  assert.ok(family.variants.every(({ checks }) => checks.some(({ expression }) => expression.includes("retrieve_memories"))));
});

test("Python Agent 交接课接入能力路由个性化练习 family", async () => {
  const { authoredCatalog } = await import("../app/content/catalog.ts");
  const lesson = authoredCatalog.tracks.find(({ id }) => id === "python")?.lessons.find(({ id }) => id === "agent-handoff");
  assert.equal(lesson?.familyId, "python-handoff-v1");
  const family = (await import("../app/exercises/families.ts")).exerciseFamilies.find(({ id }) => id === lesson.familyId);
  assert.ok(family);
  assert.equal(family.variants.length, 6);
...
test("Python DeepResearch 项目接入章节与来源个性化练习 family", async () => {
  const { authoredCatalog } = await import("../app/content/catalog.ts");
  const lesson = authoredCatalog.tracks.find(({ id }) => id === "python")?.lessons.find(({ id }) => id === "agent-deep-research-project");
  assert.equal(lesson?.familyId, "python-research-v1");
  const family = (await import("../app/exercises/families.ts")).exerciseFamilies.find(({ id }) => id === lesson.familyId);
  assert.ok(family);
  assert.equal(family.variants.length, 6);
  assert.ok(family.variants.every(({ checks }) => checks.some(({ expression }) => expression.includes("sources"))));
});

test("Python Agent 毕业项目接入工具注册与步数限制练习 family", async () => {
  const { authoredCatalog } = await import("../app/content/catalog.ts");
  const lesson = authoredCatalog.tracks.find(({ id }) => id === "python")?.lessons.find(({ id }) => id === "agent-framework-capstone");
  assert.equal(lesson?.familyId, "python-framework-capstone-v1");
  const family = (await import("../app/exercises/families.ts")).exerciseFamilies.find(({ id }) => id === lesson.familyId);
  assert.ok(family);
  assert.equal(family.variants.length, 6);
...
test("LangChain RAG 观测项目接入指标与空记录边界练习 family", async () => {
  ...
  assert.equal(lesson?.familyId, "langchain-rag-observability-v1");
  const family = (await import("../app/exercises/families.ts")).exerciseFamilies.find(({ id }) => id === lesson.familyId);
  assert.ok(family);
  assert.equal(family.variants.length, 6);
  assert.ok(family.variants.every(({ checks }) => checks.some(({ expression }) => expression.includes("average_latency_ms"))));
});
...
test("LangGraph 多 Agent 项目接入角色调度与未知角色边界练习 family", async () => {
  const { authoredCatalog } = await import("../app/content/catalog.ts");
  const lesson = authoredCatalog.tracks.find(({ id }) => id === "langgraph")?.lessons.find(({ id }) => id === "langgraph-lesson-17");
  assert.equal(lesson?.familyId, "langgraph-dispatch-project-v1");
  const family = (await import("../app/exercises/families.ts")).exerciseFamilies.find(({ id }) => id === lesson.familyId);
  assert.ok(family);
  assert.equal(family.variants.length, 6);
