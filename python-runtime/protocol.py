...
def decode_request(frame):
    ...
    if request["method"] == "tutor.plan" and (not isinstance(params["now"], str) or not params["now"]):
        raise ProtocolError("now 必须是非空字符串")
    if request["method"] == "tutor.validate" and not isinstance(params["state"], dict):
        raise ProtocolError("state 必须是对象")
    ...
    if request["method"] == "documents.save" and (not isinstance(params["documents"], list) or not params["documents"]):
        raise ProtocolError("documents.save 的 documents 必须是非空数组")
