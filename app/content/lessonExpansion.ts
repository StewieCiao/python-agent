...
const PROJECT_BRIEFS: Record<"langchain-rag" | "langgraph", ProjectBrief[]> = {
  "langchain-rag": [
    { title: "可引用文档问答系统", summary: "为团队建立可追溯的文档问答入口，回答始终带真实来源。", prompt: "实现 answer_with_sources(matches, answer)，仅当 matches 非空时返回 answer 与去重后的 sources；无命中返回 answer='资料不足' 且 sources=[]。", starterCode: "def answer_with_sources(matches, answer):\n    pass\n", solution: "def answer_with_sources(matches, answer):\n    if not matches:\n        return {\"answer\": \"资料不足\", \"sources\": []}\n    sources = []\n    for match in matches:\n        if match[\"source\"] not in sources:\n            sources.append(match[\"source\"])\n    return {\"answer\": answer, \"sources\": sources}\n", hints: ["先处理无命中边界。", "按输入顺序去重来源。", "回答和来源必须来自真实输入。"], checks: [{ name: "引用来源", expression: "answer_with_sources([{\"source\": \"a.md\"}, {\"source\": \"a.md\"}], \"ok\") == {\"answer\": \"ok\", \"sources\": [\"a.md\"]}", failure: "命中结果应保留去重后的真实来源。", kind: "behavior" }, { name: "无资料", expression: "answer_with_sources([], \"猜测\") == {\"answer\": \"资料不足\", \"sources\": []}", failure: "无命中时必须明确资料不足。", kind: "behavior" }] },
    { title: "混合检索评估台", summary: "为检索调优提供关键词、向量召回和命中率对比。", prompt: "实现 score_retrieval(expected, retrieved)，返回 recall 与 first_hit；expected 为空时抛出 ValueError，first_hit 为首个命中位置或 None。", starterCode: "def score_retrieval(expected, retrieved):\n    pass\n", solution: "def score_retrieval(expected, retrieved):\n    if not expected:\n        raise ValueError(\"expected cannot be empty\")\n    hits = [item for item in retrieved if item in expected]\n    first_hit = next((index for index, item in enumerate(retrieved) if item in expected), None)\n    return {\"recall\": len(set(hits)) / len(set(expected)), \"first_hit\": first_hit}\n", hints: ["先拒绝空的期望集合。", "recall 分母是去重后的期望来源数。", "首个命中保留列表索引。"], checks: [{ name: "召回率", expression: "score_retrieval([\"a\", \"b\"], [\"x\", \"b\"])[\"recall\"] == 0.5", failure: "recall 应反映实际命中比例。", kind: "behavior" }, { name: "首命中", expression: "score_retrieval([\"a\"], [\"x\", \"a\"])[\"first_hit\"] == 1", failure: "应记录首个真实命中位置。", kind: "behavior" }] },
    { title: "带工具调用的知识助手", summary: "为业务助手连接受约束的工具，并保留每次调用结果。", prompt: "实现 call_tool(tools, name, payload)，只调用已注册工具并返回 name、input、observation；未知工具抛出 KeyError。", starterCode: "def call_tool(tools, name, payload):\n    pass\n", solution: "def call_tool(tools, name, payload):\n    observation = tools[name](payload)\n    return {\"name\": name, \"input\": payload, \"observation\": observation}\n", hints: ["用字典键选择工具。", "把原始 payload 和真实返回值都记录下来。", "不要为未知工具伪造结果。"], checks: [{ name: "工具结果", expression: "call_tool({\"search\": lambda value: value.upper()}, \"search\", \"rag\") == {\"name\": \"search\", \"input\": \"rag\", \"observation\": \"RAG\"}", failure: "工具调用记录必须包含真实输入和输出。", kind: "behavior" }, { name: "未知工具", expression: "_raises_key_error(lambda: call_tool({}, \"missing\", \"x\"))", failure: "未知工具应保留 KeyError。", kind: "behavior" }] },
    { title: "RAG 质量观测面板", summary: "为 RAG 流程记录召回、引用和无答案边界，定位质量回归。", prompt: "实现 summarize_rag_runs(runs)，返回 runs 数量、平均 latency_ms 和资料不足次数；缺失字段抛出 KeyError。", starterCode: "def summarize_rag_runs(runs):\n    pass\n", solution: "def summarize_rag_runs(runs):\n    if not runs:\n        return {\"count\": 0, \"average_latency_ms\": 0, \"insufficient\": 0}\n    return {\"count\": len(runs), \"average_latency_ms\": sum(run[\"latency_ms\"] for run in runs) / len(runs), \"insufficient\": sum(run[\"answer\"] == \"资料不足\" for run in runs)}\n", hints: ["先定义空运行列表的统计结果。", "平均耗时使用每条真实记录。", "资料不足只按明确答案计数。"], checks: [{ name: "统计结果", expression: "summarize_rag_runs([{\"latency_ms\": 10, \"answer\": \"ok\"}, {\"latency_ms\": 20, \"answer\": \"资料不足\"}]) == {\"count\": 2, \"average_latency_ms\": 15.0, \"insufficient\": 1}", failure: "面板应根据真实运行记录计算统计。", kind: "behavior" }, { name: "空输入", expression: "summarize_rag_runs([]) == {\"count\": 0, \"average_latency_ms\": 0, \"insufficient\": 0}", failure: "没有运行记录时应返回明确的零统计。", kind: "behavior" }] },
  ],
  langgraph: [
    { title: "可恢复研究工作流", summary: "为研究任务建立可暂停、可恢复且按 thread 隔离的工作流。", prompt: "实现 resume_state(checkpoints, thread_id, updates)，复制指定 thread 状态并应用 updates；未知 thread_id 抛出 KeyError。", starterCode: "def resume_state(checkpoints, thread_id, updates):\n    pass\n", solution: "def resume_state(checkpoints, thread_id, updates):\n    state = dict(checkpoints[thread_id])\n    state.update(updates)\n    return state\n", hints: ["只读取指定 thread 的检查点。", "复制状态后再合并更新。", "未知线程保留 KeyError。"], checks: [{ name: "恢复状态", expression: "resume_state({\"t1\": {\"step\": 2}}, \"t1\", {\"step\": 3}) == {\"step\": 3}", failure: "恢复应基于同一 thread 的状态。", kind: "behavior" }, { name: "线程隔离", expression: "resume_state({\"a\": {\"value\": 1}, \"b\": {\"value\": 2}}, \"b\", {}) == {\"value\": 2}", failure: "不能读取其他 thread 的状态。", kind: "behavior" }] },
    { title: "人工审核 Agent 流程", summary: "为高风险动作加入人工审核节点，拒绝时留下明确状态。", prompt: "实现 review_gate(action, decision)，approve 返回 action，reject 返回 status='cancelled' 的字典，其他决定抛出 ValueError。", starterCode: "def review_gate(action, decision):\n    pass\n", solution: "def review_gate(action, decision):\n    if decision == \"approve\":\n        return {\"status\": \"approved\", \"action\": action}\n    if decision == \"reject\":\n        return {\"status\": \"cancelled\", \"action\": action}\n    raise ValueError(\"unknown decision\")\n", hints: ["批准和拒绝都是显式状态。", "保留待审核动作。", "未知决定不能默认批准。"], checks: [{ name: "批准", expression: "review_gate(\"send\", \"approve\") == {\"status\": \"approved\", \"action\": \"send\"}", failure: "批准应返回可观察状态。", kind: "behavior" }, { name: "拒绝", expression: "review_gate(\"send\", \"reject\")[\"status\"] == \"cancelled\"", failure: "拒绝必须停止动作并留下状态。", kind: "behavior" }] },
    { title: "多 Agent 协作调度器", summary: "为多角色 Agent 设计可观察的交接和 supervisor 路由。", prompt: "实现 dispatch(tasks, workers)，按 tasks 顺序把每个 task 交给 workers[task['role']]，返回包含 role、result 的列表；未知角色抛出 KeyError。", starterCode: "def dispatch(tasks, workers):\n    pass\n", solution: "def dispatch(tasks, workers):\n    results = []\n    for task in tasks:\n        result = workers[task[\"role\"]](task[\"input\"])\n        results.append({\"role\": task[\"role\"], \"result\": result})\n    return results\n", hints: ["路由键来自每个任务的 role。", "保留任务顺序和真实 worker 返回值。", "未知角色保留 KeyError。"], checks: [{ name: "顺序调度", expression: "dispatch([{\"role\": \"search\", \"input\": \"x\"}], {\"search\": lambda value: value + \"!\"}) == [{\"role\": \"search\", \"result\": \"x!\"}]", failure: "调度器应调用对应 worker 并保留结果。", kind: "behavior" }, { name: "未知角色", expression: "_raises_key_error(lambda: dispatch([{\"role\": \"write\", \"input\": \"x\"}], {}))", failure: "未知角色不能静默跳过。", kind: "behavior" }] },
    { title: "带长期记忆的任务图", summary: "为用户偏好建立跨 thread 的 Store，并与短期状态分离。", prompt: "实现 read_memory(store, user_id, key)，只从 (user_id,) 命名空间读取 key；不存在返回 None，不得读取其他用户。", starterCode: "def read_memory(store, user_id, key):\n    pass\n", solution: "def read_memory(store, user_id, key):\n    return store.get((user_id,), {}).get(key)\n", hints: ["命名空间第一项必须是当前 user_id。", "先取得命名空间中的字典。", "不存在时保留 None。"], checks: [{ name: "用户隔离", expression: "read_memory({(\"u1\",): {\"theme\": \"dark\"}}, \"u1\", \"theme\") == \"dark\"", failure: "应读取当前用户的长期记忆。", kind: "behavior" }, { name: "未知用户", expression: "read_memory({(\"u1\",): {\"theme\": \"dark\"}}, \"u2\", \"theme\") is None", failure: "不能跨用户读取记忆。", kind: "behavior" }] },
  ],
};

const FRAMEWORK_VIDEOS: Record<"langchain-rag" | "langgraph", CourseVideo[]> = {
  "langchain-rag": [
    { title: "LangChain for LLM Application Development", url: "https://www.deeplearning.ai/short-courses/langchain-for-llm-application-development/", provider: "DeepLearning.AI", language: "英文", duration: "短课程", note: "补充模型、Prompt 与 Runnable 的完整数据流。" },
    { title: "LangChain: Chat with Your Data", url: "https://www.deeplearning.ai/short-courses/langchain-chat-with-your-data/", provider: "DeepLearning.AI", language: "英文", duration: "短课程", note: "补充文档加载、切分、检索与引用。" },
  ],
  langgraph: [
    { title: "Introduction to LangGraph", url: "https://academy.langchain.com/courses/intro-to-langgraph", provider: "LangChain Academy", language: "英文", duration: "课程", note: "补充 StateGraph、节点、边和执行模型。" },
    { title: "LangGraph Essentials", url: "https://academy.langchain.com/courses/langgraph-essentials-python", provider: "LangChain Academy", language: "英文", duration: "课程", note: "补充持久化、中断、恢复与多 Agent。" },
  ],
};
...
const PYTHON_TOPIC_SPECS: Record<string, TopicSpec> = {
  ...
  "HTTP 请求与失败边界": {
    summary: "把 HTTP 状态、超时和响应正文作为显式结果处理，不伪造成功。",
    prompt: "实现 classify_response(status, body, timed_out)，返回 status 为 ok、http_error 或 timeout 的字典；超时优先，2xx 才算 ok。",
    starterCode: "def classify_response(status, body, timed_out):\n    pass\n",
    solution: "def classify_response(status, body, timed_out):\n    if timed_out:\n        return {\"status\": \"timeout\", \"body\": None}\n    if not 200 <= status < 300:\n        return {\"status\": \"http_error\", \"body\": body}\n    return {\"status\": \"ok\", \"body\": body}\n",
    hints: ["先处理真实 timeout 状态。", "再按 2xx 区分成功和 HTTP 错误。", "保留错误响应正文，避免返回空成功结果。"],
    checks: [
...
function generatedLesson(track: CourseTrack, index: number, stageId: string, project: boolean, familyId?: string): CourseLesson {
  ...
  return {
    ...
    tags: [track.id, `stage-${stageId}`], guide: [
      ...
      { kind: "常见误区", title: `${baseTopic}的边界检查`, body: `不要只复现${scenario}的示例；使用未出现在题面中的输入，观察失败属于行为不符还是缺少教学构造。`, bullets: ["换一组输入", "保留真实输出", "解释期望与实际"], example: `检查：${checkNames}` },
...
export function expandCourseTrack(track: CourseTrack, expansion: Expansion): CourseTrack {
  ...
  for (const lesson of lessons) {
    ...
    lesson.exercise.prompt = `阶段项目：${lesson.title}\n用户场景：${projectStory}\n用户故事：交付一个能被真实使用者复现和演示的最小版本。\n输入与输出：先写清数据契约；失败状态：保留真实异常或无结果状态；验收：补充典型、变化和边界测试，并在 README 记录运行方式、取舍与限制。${brief ? `\n最小可运行里程碑：${brief.prompt}` : ""}`;
    if (brief) {
      lesson.exercise.starterCode = brief.starterCode;
      lesson.exercise.solution = brief.solution;
      lesson.exercise.hints = brief.hints;
      lesson.browserChecks = brief.checks;
    } else {
