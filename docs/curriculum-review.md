...
- 个性化练习 family 现在从作者源携带六组变体和独立行为检查；覆盖 Python 输出/循环/字符串/分支/函数/容器/异常/类与对象/生成器/装饰器/项目汇总/Tool Registry/Action 解析，以及 LangChain 结构化输出、检索链、混合检索、重排、引用、RAG 评估和 LangGraph 路由、checkpoint、Store 长期记忆、Interrupt 审批、人工审核。Python 服务只校验签名快照中的检查，桌面 Worker 使用个性题检查而不是原关卡样例。
