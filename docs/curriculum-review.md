...
- 桌面端 RAG 资料现在可由用户显式保存到 SQLite 本地资料库，并按正文 SHA-256 去重；读取资料库仍需用户主动点击，不会把文件内容自动写入聊天历史或发送到浏览器存储。
- 资料库提供显式清空操作，只删除 `rag_documents` 表中的资料片段，保留学习进度、聊天记录和模型配置。
...
- 个性题入口使用本机随机学习者标识派生稳定种子；标识只用于选择 family 变体，不包含代码、密钥或课程数据，也不写入 Python 服务。
- 项目个性化覆盖回归已锁定 14 个阶段项目（Python 6、LangChain/RAG 4、LangGraph 4）全部绑定 family；每个 family 固定六组变体并至少包含两项行为检查，防止新增项目只完成静态题面而遗漏错题闭环。
- 项目时长回归要求所有 14 个项目至少预留 60 分钟实践时间；综合项目可超过该下限，但不会被普通课程的短时长覆盖。
- 简历项目可通过 `npm run demo:projects` 使用仓库内置 Python 运行时一次运行，任一演示失败都会保留退出码并停止。
...
- `git diff --check`：通过。
- 项目自带 Python 3.13.15 运行环境准备完成；内置运行时 `python-runtime` 63 项测试与 packaged smoke 均通过（LangChain 1.2.12、LangGraph 1.1.2、pypdf 6.16.2、SQLite FTS5）。测试包含 RAG 空问题、资料库保存/去重/清空、重排 top_k、Agentic RAG 无命中、Supervisor 角色失败、个性题重复轮换边界，打包复制清单包含 Tutor 图所需模块，桌面包可完成真实健康握手。
...
- 框架路线现在通过显式主题映射组织 Stage：LangChain/RAG 依次覆盖模型与提示、Runnable、文档处理、向量检索、RAG 生成、评估观测和项目；LangGraph 依次覆盖图基础、路由、持久化、长期记忆、中断恢复、多 Agent 和综合项目。项目节点按领域阶段归属，不再由生成顺序偶然分配。
- 框架课程的先修关系会过滤掉跨阶段倒置依赖，并保留真实的跨路线先修；作者目录测试确认每个内部先修都位于当前课程之前，避免“早期课程依赖后续课”。
