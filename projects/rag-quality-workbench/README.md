...
4. 用一条没有命中的问题验证返回 `status=no_results`、空来源，并确认不会调用生成步骤。
5. 将成功与失败结果写入 README 或终端记录，说明本次改动的取舍。

## 架构与验收

`load → split → embed → store` 是可重跑的离线流程；`retrieve → threshold → cite → evaluate` 是在线请求中的可观察流程。验收至少覆盖：典型命中、不同来源顺序、重复来源去重、无命中和模型失败。每次结果携带课程哈希、资料哈希和 Embedding 模型名，便于复现。
